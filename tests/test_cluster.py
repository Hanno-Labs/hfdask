...
def test_heterogeneous_groups(monkeypatch):
    ...
    submit_cluster(JobSpec("example", "image", "workload:run"), keys, public_relays=True,
    ...
    assert [call.kwargs["flavor"] for call in api.run_job.call_args_list] == [
        "cpu-basic", "cpu-performance", "cpu-performance", "a100-large", "a100-large", "a100-large"]
...
@pytest.mark.parametrize("scheduler,worker,expected", [
...
def test_hardware_overrides(monkeypatch, colocated, scheduler, worker, expected):
    ...
    submit_cluster(JobSpec("example", "image", "workload:run", flavor="cpu-upgrade"),
                   keys, public_relays=True, scheduler_worker=colocated,
                   scheduler_flavor=scheduler, worker_flavor=worker, api=api)
...
def test_scheduler_worker_reduces_jobs(monkeypatch, workers):
    ...
    cluster = submit_cluster(JobSpec("example", "image", "workload:run", workers=workers),
                             keys, public_relays=True, scheduler_worker=True, api=api)
...
def test_partial_launch_retains_handles(monkeypatch):
    ...
    with pytest.raises(LaunchError) as caught:
        submit_cluster(JobSpec("example", "image", "workload:run", workers=1),
                       [Identity(b"a" * 32), Identity(b"b" * 32)], public_relays=True, api=api)
