...
def test_boot_and_restore(monkeypatch):
    ...
    snapshots = []
    cluster = boot_cluster(JobSpec("example", "image", workers=2), nodes,
                           client_identity=external, public_relays=True,
                           scheduler_worker=True, api=api,
...
def test_boot_rejects_workload():
    with pytest.raises(ValueError, match="entrypoint"):
        boot_cluster(JobSpec("example", "image", "workload:run"), [],
                     client_identity=Identity(b"a" * 32))
...
def test_client_connect_disconnect_keeps_scheduler(monkeypatch):
    ...
    manifest = {"connection": {"schema": 1, "persistent": True, "job_nodes": 1,
                               "peers": ["server", "client"], "scheduler_worker": True,
                               "public_relays": True, "relays": []}}
    with LocalCluster(n_workers=1, threads_per_worker=1, processes=False,
                      host="127.0.0.1", scheduler_port=21000, dashboard_address=None,
                      startup_information={"hfdask": lambda worker: {
                          "node": 0, "workers_on_node": 1}}) as cluster:
        for _ in range(2):
...
def persistent_server():
    ...
    config = {"job_nodes": 1, "node_flavors": ["cpu-basic"], "node_tags": [[]],
              "startup_timeout": 20, "persistent": True}
    args = Namespace(node=0, scheduler_worker=True, threads_per_worker=1,
                     memory_limit="256MiB", entrypoint="missing:must_not_run")
...
"""
    try:
        ...
        subprocess.run([sys.executable, "-c", ("from distributed import Client; "
...
def encrypted_server(ready):
    ...
    async def serve():
        ...
        endpoint = await iroh.Endpoint.bind(iroh.EndpointOptions(
            preset=iroh.preset_minimal(), secret_key=b"a" * 32,
            bind_addr="127.0.0.1:23111", alpns=[ALPN]))
        peers = [endpoint.addr(), iroh.EndpointAddr(
            iroh.SecretKey.from_bytes(b"b" * 32).public(), None, [])]
        try:
            async with Mesh(endpoint, peers, 0, services=[0, 0], bind_host="127.0.0.2"):  # noqa: SIM117
                async with Scheduler(host="127.0.0.2", port=21000,
                                     dashboard_address=None) as scheduler:
                    async with Worker(scheduler.address, host="127.0.0.2", port=21001,
                                      contact_address="tcp://127.0.0.1:21001",
                                      nthreads=1, dashboard_address=None,
                                      startup_information={"hfdask": lambda w: {
                                          "node": 0, "workers_on_node": 1}}):
                        ready.set()
...
def test_real_encrypted_background_client(monkeypatch):
    ...
    async def local_bind(options):
        return await original_bind(iroh.EndpointOptions(
            preset=iroh.preset_minimal(), secret_key=options.secret_key,
            bind_addr="127.0.0.1:0", alpns=options.alpns))
    ...
    monkeypatch.setattr(iroh, "EndpointAddr", lambda key, relay, addresses: original_addr(
        key, None, ["127.0.0.1:23111"] if key.to_bytes() ==
        iroh.SecretKey.from_bytes(b"a" * 32).public().to_bytes() else []))
    manifest = {"connection": {"schema": 1, "persistent": True, "job_nodes": 1,
        "peers": [Identity(b"a" * 32).public_id(), Identity(b"b" * 32).public_id()],
        "scheduler_worker": True, "public_relays": True, "relays": []}}
    try:
