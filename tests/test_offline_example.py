...
def example(monkeypatch):
    ...
    class Engine:
        ...
        def generate(self, prompts, params, use_tqdm):
            ...
            return [
                SimpleNamespace(
                    outputs=[] if state.reply is None else [SimpleNamespace(text=state.reply)]
                )
                for _ in prompts
            ]

    monkeypatch.setitem(
        sys.modules, "vllm", SimpleNamespace(LLM=Engine, SamplingParams=lambda **kwargs: kwargs)
    )
    ...
    state.worker = SimpleNamespace(
        ...
        state=SimpleNamespace(total_resources={"GPU": 1}),
...
def source():
    return pd.DataFrame(
        {"text": [f"article {i}" for i in range(200)], "label": [i // 50 for i in range(200)]},
        index=range(1000, 1200),
    )
...
def test_main_mounted_dataset_partitioning_and_summary(
    ...
    worker_specs = {
        ...
        **{
            f"gpu-{i}": {"cls": Worker, "options": {"nthreads": 1, "resources": {"GPU": 1}}}
    ...
    with (
        SpecCluster(
            ...
            scheduler={"cls": Scheduler, "options": {"dashboard_address": None}},
        ) as cluster,
        Client(cluster) as client,
    ):
        client.wait_for_workers(1 + gpu_workers, timeout=10)
        workers = client.scheduler_info()["workers"]
        cpus = {address for address, info in workers.items() if not info["resources"].get("GPU", 0)}
    ...
    for address in gpus:
        model_events = [
            event
            for event in events
            if event["worker"] == address and event["phase"].startswith("model_")
        ]
    ...
    pd.testing.assert_frame_equal(
        ...
        job.select_sample(source).assign(prediction="World"),
        check_dtype=False,
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ["World", "Sports", "world", "", None, pd.NA],
            ["World", "World", "World", "World", "World", "World"],
            4,
            1 / 6,
            id="mixed-invalid-and-missing",
        ),
        pytest.param(
            ["World", "Sports", pd.NA],
            ["World", pd.NA, pd.NA],
            1,
            1 / 3,
            id="missing-labels-count-as-incorrect",
        ),
        pytest.param(["world", "", None], ["World", "World", "World"], 3, 0.0, id="all-invalid"),
        pytest.param([], [], 0, None, id="empty"),
    ],
)
def test_save_results_metrics_and_parquet(
    example, tmp_path, capsys, predictions, labels, invalid, accuracy
):
    job, state = example
    ...
    assert capsys.readouterr().out == ""
    pd.testing.assert_frame_equal(
        pd.read_parquet(output / "results.parquet"), results.reset_index(drop=True)
    )
