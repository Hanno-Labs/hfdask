...
import dask
import pandas as pd
from distributed import WorkerPlugin, get_client, get_worker
...
class WorkerSetup(WorkerPlugin):
    def setup(self, worker):
        ...
        worker.inference_model = LLM(
            ...
            max_model_len=2048, max_num_seqs=16, gpu_memory_utilization=0.5,
            enforce_eager=True, trust_remote_code=False, seed=23,
        )
        worker.log_event("inference", {
            "phase": "model_loaded", "elapsed": time.monotonic() - started,
        })
...
def infer(partition: pd.DataFrame) -> pd.DataFrame:
    ...
    prompts = [tokenizer.apply_chat_template(
        [{"role": "user", "content": "Classify this news article. Reply with exactly one "
    ...
    outputs = engine.generate(
        prompts, SamplingParams(temperature=0, max_tokens=16, seed=23), use_tqdm=False,
    )
    result["prediction"] = [output.outputs[0].text.strip() if output.outputs else ""
                            for output in outputs]
    worker.log_event("inference", {
        "phase": "partition_complete", "rows": len(partition),
        "elapsed": time.monotonic() - started,
    })
    return result


def save_results(results: pd.DataFrame, output: Path) -> dict:
...
def main() -> None:
    ...
    cpu_workers = [address for address, info in workers.items()
